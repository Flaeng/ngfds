...
async function getDocumentationAsync(file) {
  ...
  const newSourceFolderPath =
    './projects/ngfds/src/lib/' +
    split
      .slice(indexOfPage + 1)
  ...
  for (const declartion of sourceCode.declarations) {
    const decl =
      declartion instanceof ClassDeclaration ||
      declartion instanceof InterfaceDeclaration
        ? declartion
        : null;

    if (decl) {
      if (decl.isExported === false) continue;
      ...
      result.push(
        `<fds-card header="${decl.name}" subheader="${typeName}" id="${decl.name}" class="mb-5" style="display:block"><fds-card-content>`
      );
      // result.push(`<h3>${decl.name}</h3>`);

      const propertyList = decl.properties.filter((x) => x.visibility === 2);
      const accessorList = decl.accessors
        .filter((x) => x.visibility === 2)
        .groupBy((x) => x.name)
        .map((x) => {
          const res = {
            name: x[0].name,
      ...
      if (propList.length !== 0) {
        ...
        result.push(
          `<table class="table mb-5 table--zebra table--compact table--responsive-scroll">`
        );
        result.push(`<tr>`);
        result.push(
          `<th fds-tooltip="Property name when referencing the component in TypeScript">Name</th>`
        );
        result.push(
          `<th fds-tooltip="Attribute to use on the component">Attribute</th>`
        );
        result.push(`<th fds-tooltip="Property type(s)">Type</th>`);
        result.push(
          `<th fds-tooltip="Default value of the property">Default Value</th>`
        );
        ...
        for (const prop of propList) {
          let propSource =
            prop.start && prop.end
              ? content.substring(prop.start, prop.end)
              : null;
          ...
          if (input && input.length != 0) {
            ...
            const domRef = match.substring(
              indexOfPararentesStart + 2,
              indexOfPararentesEnd - 1
            );
          ...
          const typeList = prop.type
            ? prop.type.split('|').map((x) => x.trim())
            : [];
          const newTypeList = [];
          for (const param of typeList) {
            ...
            const declCandidates = sourceCode.declarations.filter(
              (x) => x.name === typeName
            );
      ...
      if (methodList.length !== 0) {
        ...
        for (const prop of methodList) {
          ...
          if (param.length !== 0) result.push(`<td>${param}</td>`);
          else result.push(`<td>(none)</td>`);
...
async function formatFilecontentAsync(filename, content) {
  ...
  for (const line of lineList) {
    ...
    if (emptyDemoViewTagContent) {
      ...
      demoViewTagContent = demoViewTagContent.map(
        (x) => x.substring(12) + '<br />'
      );
...
if (!getArgument('watch')) {
  glob(filepathPattern, (err, matches) => {
